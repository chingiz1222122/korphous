...
from app.memory import process_message_for_memory
...
class Collector:
    ...
    async def handle_new_message(self, event: events.NewMessage.Event) -> None:
        try:
            ...
            if saved:
                self.db.set_state("collector_last_ingest_utc", str(int(time.time())))
                process_message_for_memory(
                    self.db,
                    chat_id=chat.id,
                    msg_id=event.id,
                    date_utc=event.message.date.astimezone(timezone.utc).isoformat(),
                    text=(event.message.message or "").strip(),
                )
    ...
    async def run(self) -> None:
        ...
        self.db.set_state("collector_status", "running")
        self.db.set_state("collector_started_utc", str(int(time.time())))
    ...
    async def _heartbeat_loop(self) -> None:
        while True:
            self.db.update_heartbeat("collector")
